 ...
 *   select:SELECTOR|VALUE  -> choose option in dropdown
 *   check:SELECTOR         -> check a checkbox/radio
 *   uncheck:SELECTOR       -> uncheck a checkbox
 *   wait:SELECTOR_OR_MS    -> wait for selector or milliseconds
  ...
  for (const raw of steps || []) {
    ...
    try {
      if (action === "go") {
        ...
        const [sel, value] = arg.split("|", 2);
        if (sel && value !== undefined) {
          await page.locator(sel.trim()).selectOption(value.trim());
        } else {
          log.warn(`Malformed select step: ${raw}`);
        }
      } else if (action === "check") {
        await page.locator(arg.trim()).check();
      } else if (action === "uncheck") {
        await page.locator(arg.trim()).uncheck();
      } else if (action === "wait") {
        const ms = Number(arg);
        if (!Number.isNaN(ms)) {
