...
const BASE = "https://www.demoblaze.com";

export async function runDemoblazeFlow({ page, goal, llmChooseFn }) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const href = (await tile.locator(S.productNameInTile).first().getAttribute("href").catch(() => null)) || null;

    const detailsLink = href ? new URL(href, BASE).toString() : null;
    const priceNumber = parsePriceToNumber(priceRaw);

    if (name && detailsLink) items.push({ name, priceRaw, priceNumber, detailsLink });
  }
